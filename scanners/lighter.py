...
class LighterScanner(BaseScanner):
    ...
    async def get_funding_rates(self) -> list[FundingRate]:
        ...
        # Lighter API возвращает {"code": 200, "funding_rates": [...]}
        # Каждый элемент содержит поле "exchange" (binance/bybit/hyperliquid/lighter)
        ...
        for item in items:
            ...
            try:
                # Lighter API отдаёт 8-часовую ставку в долях (формат как Binance/Bybit)
                # Сайт подтверждает: 1hr=-0.0192%, 8hr=-0.1536%, Ann=-168.19%
                # Значит API rate = 8hr fraction → делим на 8 для часовой ставки
                rate_8h = float(
                    item.get("rate") or
