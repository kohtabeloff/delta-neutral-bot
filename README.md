...
- Аккаунты на биржах: Backpack, Lighter, Variational, Extended
...
> ⚠️ `vr-token` — сессионный токен браузера. Он истекает через некоторое время. При ошибке авторизации нужно обновить его в `.env` и перезапустить бота.
...
LIGHTER_API_KEY_INDEX=2     # индекс ключа (тот что вводил при создании)
LIGHTER_ACCOUNT_INDEX=      # Account Index твоего аккаунта на Lighter
